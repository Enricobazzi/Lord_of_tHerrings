...
eco_df <- read.table(
  file = paste0("data/angsd_matrix/dapc/stocks_table.",
                pcangsd_dataset, ".", dapc_dataset, ".csv"),
  header = T, sep = ","
)

summary_table <- eco_df |>
  count(Period, x, y, Stock, name = "n") |>
  pivot_wider(
    names_from = Stock,
    values_from = n,
    values_fill = 0
...
pie_data <- summary_table |>
  ...
  group_walk(~ {
    ...
    p <- ggplot(.x, aes(x = 1, y = Count, fill = Stock)) +
      geom_col(width = 0.1, color = "grey20", linewidth = 0.1) +
    ...
    ggsave(
      paste0(
        "plots/angsd_matrix/dapc/pie_charts/Period_", .y$Period,
        "_X_", .y$x,
        "_Y_", .y$y,
        ".png"
      ),
...
for (tp in unique(eco_df$Period)){
  ...
  basemap2 <- basemap +
    geom_point(
      data = st, aes(x = x, y = y), shape = 1, size = 0.5, color = "grey20"
    ) +
    coord_sf(
  ...
  for (i in seq_len(nrow(st))) {
    ...
    # add pie charts
    filename <- paste0(
      "plots/angsd_matrix/dapc/pie_charts/Period_", tp,
      "_X_", x_val,
      "_Y_", y_val,
      ".png"
    )
...
ggsave(
  plot = legend,
  filename = paste0("plots/angsd_matrix/dapc/Stocks_legend.png"),
  width = 90, height = 90, unit = "mm", dpi = 600, device = png
)
